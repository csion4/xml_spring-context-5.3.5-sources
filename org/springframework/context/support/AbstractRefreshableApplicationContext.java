...
public abstract class AbstractRefreshableApplicationContext extends AbstractApplicationContext {
	...
	protected void customizeBeanFactory(DefaultListableBeanFactory beanFactory) {
		if (this.allowBeanDefinitionOverriding != null) {
			beanFactory.setAllowBeanDefinitionOverriding(this.allowBeanDefinitionOverriding);  // 设置bean是否允许覆盖，也就是说在出现解析出多个bean的id或name相同时，是否允许后面的覆盖前面的，默认是false这样在出现多个相同bean时spring会报错
		}
		if (this.allowCircularReferences != null) {
			beanFactory.setAllowCircularReferences(this.allowCircularReferences); // 是否允许循环依赖，默认allowCircularReferences = true，如果设置为不支持，则会在org.springframework.beans.factory.support.AbstractAutowireCapableBeanFactory.doCreateBean方法中不放入三级缓存
		}
	}
